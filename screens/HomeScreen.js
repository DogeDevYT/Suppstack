...
const HomeScreen = ({ navigation, session }) => {
  ...
  if (loading) return <SafeAreaView style={styles.container}><ActivityIndicator size="large" color="#1e40af" style={{ flex: 1 }} /></SafeAreaView>;
  if (fetchError) return (
    <SafeAreaView style={styles.container}><View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }}><Text style={{ textAlign: 'center', color: '#ef4444', marginBottom: 20 }}>{fetchError}</Text><TouchableOpacity style={styles.button} onPress={() => supabase.auth.signOut()}><Text style={styles.buttonText}>Go to Login</Text></TouchableOpacity></View></SafeAreaView>
  );
        ...
        renderItem={({ item }) => (
          <View>
            <SupplementCard
              ...
              dosage={item.dosage}
              // --- KEY CHANGE ---
              // Pass all necessary data to the detail screen
              onPress={() => navigation.navigate('SupplementDetail', {
                supplementId: item.supplement_id,
                stackItemId: item.id,
                currentDosage: item.dosage,
                currentImageUrl: item.image_url, // Pass the user's specific image URL
                session: session,
              })}
            />
            <TouchableOpacity style={styles.removeButton} onPress={() => handleRemoveSupplement(item.id)}><Text style={styles.removeButtonText}>Remove</Text></TouchableOpacity>
          </View>
        )}
        style={{ paddingHorizontal: 20 }}
        ListEmptyComponent={<Text style={{ textAlign: 'center', color: '#6b7280', marginTop: 40 }}>Your stack is empty.</Text>}
      />
      <View style={{ paddingHorizontal: 20, paddingBottom: 20, paddingTop: 10 }}>
        <TouchableOpacity style={[styles.button, styles.addButton]} onPress={() => navigation.navigate('AddSupplement')}><Text style={styles.buttonText}>+ Add Supplement</Text></TouchableOpacity>
      </View>
    </SafeAreaView>
  );
};
